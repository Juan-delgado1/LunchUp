...
public class DisjointSet {    
    ...
    // find() O(1): Encontrar el representante (raíz) del conjunto al que pertenece el elemento x
    ...
    // union(x, y)  O(1): Unir dos conjuntos utilizando Union by Rank
    public void union(int x, int y) {

        // Se ncuentra los representantes de los conjuntos 
        int parentA = find(x);
        int parentB = find(y);

        // Si los conjuntos son diferentes se realiza una unión,
        //sino no es necesario
        if (parentA != parentB) {

            // Si el rango de parentA es menor que el rango de parentB,
            //hace a parentB padre de parentA
            if (rank[parentA] < rank[parentB]) {
                parent[parentA] = parentB;


            } else if (rank[parentA] > rank[parentB]) { // Sino es así, 
                                                        //se hace al contrario
                parent[parentB] = parentA;

            } else { // Si ambos rangos son iguales, 
                     //se incrementa el rando de quien va a ser representante
                parent[parentB] = parentA;
                rank[parentA]++;
            }
    ...
    public static void main(String[] args) {
        ...
        DisjointSet conjuntos = new DisjointSet(n);          
